...
def Divider_eqn(pt1,pt2,vehicle):
    ...
    pos_last = vehicle.positions[-1] # latest position
    pos_othr = vehicle.positions[-4] # some previous position
    print(pos_last,pos_othr)
...
class VehicleCounter(object):
    def __init__(self, shape, divider1, divider2, divider3, divider4):
        ...
        self.vehicles = []
        self.traffic_count = np.zeros((4, 4, 2)) # for each divider 1 for up count and 0 for down count
        print("this car counter in init: ",self.traffic_count)
    ...
    def update_count(self, matches, output_image = None):
        ...
        for match in matches:
            ...
            new_vehicle.vehicle_class_id = match[2]
        ...
        for vehicle in self.vehicles:
            class_id = math.floor(vehicle.vehicle_class_id)
            print("in vehicles: ",self.traffic_count)
            ...
            if not vehicle.counted3 and len(vehicle.positions) > 6 and (self.divider3b_x < vehicle.positions[-1][0] < self.divider3a_x):
                res = Divider_eqn((self.divider3b_x, self.divider3b_y),(self.divider3a_x, self.divider3a_y),vehicle)
                if(res):
                    if(res>0):
                        print("up right here")
                        self.traffic_count[2][class_id-1][1] += 1
                        # self.vehicle_count3_up += 1
                        vehicle.counted3 = True
                    else:
                        # self.vehicle_count3_down += 1
                        self.traffic_count[2][class_id-1][0] += 1
            ...
            if not vehicle.counted2 and len(vehicle.positions) > 6 and (self.divider2b_x < vehicle.positions[-1][0] < self.divider2a_x):
                res = Divider_eqn((self.divider2b_x, self.divider2b_y),(self.divider2a_x, self.divider2a_y),vehicle)
                if(res):
                    if(res>0):
                        print("up right here")
                        self.traffic_count[1][class_id-1][1] += 1
                        # self.vehicle_count2_down += 1
                        vehicle.counted2 = True
                    else:
                        # self.vehicle_count2_up += 1
                        self.traffic_count[1][class_id-1][0] += 1
                        print("down right here")
                        vehicle.counted2 = True 
            ...
            if not vehicle.counted1 and len(vehicle.positions) > 6 and (self.divider1b_x < vehicle.positions[-1][0] < self.divider1a_x):
                res = Divider_eqn((self.divider1b_x, self.divider1b_y),(self.divider1a_x, self.divider1a_y),vehicle)
                if(res):
                    if(res>0):
                        # print("inside right one up: ",self.traffic_count)
                        print("up right divider here: {}".format(class_id))
                        self.traffic_count[0][class_id-1][0] += 1
                        # self.vehicle_count1_down += 1
                        vehicle.counted1 = True
                    else:
                        # print("inside right one down: ",self.traffic_count)
                        # self.vehicle_count1_up += 1
                        self.traffic_count[0][class_id-1][1] += 1
                        print("down right divider here: {}".format(class_id))
                        vehicle.counted1 = True
            # For divider 4 ROUND ABOUT
            if not vehicle.counted4 and len(vehicle.positions) > 5 and (self.divider4a_y < vehicle.positions[-1][1] < self.divider4b_y):
                res = Divider_eqn((self.divider4b_x, self.divider4b_y),(self.divider4a_x, self.divider4a_y),vehicle)
                if(res):
                    if(res>0):
                        print("up right here")
                        self.traffic_count[3][class_id-1][1] += 1
                        # self.vehicle_count4_right += 1
                        vehicle.counted4 = True
                    else:
                        self.traffic_count[3][class_id-1][0] += 1
        ...
        if output_image is not None:
            ...
            st = 0
            for v_id,veh in enumerate(item):
                cv2.putText(output_image, (veh + " leftDown:%02d" % self.traffic_count[1][v_id][0]), (250, 180 + st)
                , cv2.FONT_HERSHEY_PLAIN, 1, (127, 255, 255), 2)
                cv2.putText(output_image, (veh + " leftUp:%02d" % self.traffic_count[1][v_id][1]), (250, 200 + st)
                , cv2.FONT_HERSHEY_PLAIN, 1, (127, 255, 255), 2)
                st += 40
            ...
            st = 0
            for v_id,veh in enumerate(item):
                cv2.putText(output_image, (veh + " midDown:%02d" % self.traffic_count[2][v_id][0]), (100, 180 + st)
                , cv2.FONT_HERSHEY_PLAIN, 1, (127, 255, 255), 2)
                cv2.putText(output_image, (veh + " midUp:%02d" % self.traffic_count[2][v_id][1]), (100, 200 + st)
                , cv2.FONT_HERSHEY_PLAIN, 1, (127, 255, 255), 2)
                # cv2.putText(output_image, (veh + " rightDown:%02d" % self.traffic_count[0][v_id][0]), (400, 100 + st)
                # , cv2.FONT_HERSHEY_PLAIN, 1, (55, 255, 55), 2)
                # cv2.putText(output_image, (veh + " rightUp:%02d" % self.traffic_count[0][v_id][1]), (400, 120 + st)
                # , cv2.FONT_HERSHEY_PLAIN, 1, (55, 255, 55), 2)
                st += 40
            st = 0
            for v_id,veh in enumerate(item):
                # cv2.putText(output_image, (veh + " midDown:%02d" % self.traffic_count[2][math.floor(vehicle.vehicle_class_id)-1][0]), (400, 180 + st)
                # , cv2.FONT_HERSHEY_PLAIN, 1.7, (127, 255, 255), 2)
                # cv2.putText(output_image, (veh + " midUp:%02d" % self.traffic_count[2][math.floor(vehicle.vehicle_class_id)-1][1]), (400, 230 + st)
                # , cv2.FONT_HERSHEY_PLAIN, 1.7, (127, 255, 255), 2)
                cv2.putText(output_image, (veh + " rightDown:%02d" % self.traffic_count[0][v_id][0]), (400, 100 + st)
                , cv2.FONT_HERSHEY_PLAIN, 1, (55, 255, 55), 2)
                cv2.putText(output_image, (veh + " rightUp:%02d" % self.traffic_count[0][v_id][1]), (400, 120 + st)
            ...
            for v_id,veh in enumerate(item):
                ...
                # cv2.putText(output_image, (veh + " midUp:%02d" % self.traffic_count[2][math.floor(vehicle.vehicle_class_id)-1][1]), (400, 230 + st)
                # , cv2.FONT_HERSHEY_PLAIN, 1.7, (127, 255, 255), 2)
                cv2.putText(output_image, (veh + " BuildingIn:%02d" % self.traffic_count[3][v_id][0]), (600, 100 + st)
                , cv2.FONT_HERSHEY_PLAIN, 1, (55, 255, 55), 2)
                cv2.putText(output_image, (veh + " BuildingOut:%02d" % self.traffic_count[3][v_id][1]), (600, 120 + st)
                , cv2.FONT_HERSHEY_PLAIN, 1, (55, 255, 55), 2)
                st += 40
